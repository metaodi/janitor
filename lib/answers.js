...
function nextCalendarEntries(bot, message) {
    var matches = XRegExp.exec(message.text, XRegExp('((?<tesla>nikola|tesla)|(?<turing>alan|turing)|(?<ada>ada|lovelace)|(?<arena>arena)|(?<jazz>jazz)|(?<massage>massage))', 'i'));
    ...
    Async.each(rooms, function(room, callback) {
        cal.currentCalendarEntry(room, function(err, event, calendar) {
            ...
            if (_.isUndefined(event)) {
                msg += ':white_check_mark: ' + calendar.summary + ' free for at least 1 hour' + "\n";
                ...
                msg += ':white_check_mark: ' + calendar.summary + ' free for at least 1 hour, next event: ';
                msg += format.calendarEntry(event) + "\n";
            }
            callback();
        });
...
function uptime(bot, message) {
    ...
    msg += '- next public transport connections anywhere in Switzerland' + "\n";
    msg += '- station board for a public transport stop (think "Time For Coffee")' + "\n";
    msg += '- meeting room calendars' + "\n";
